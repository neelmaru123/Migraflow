"""
Unit Tests for Asynchronous Initial Migration Plan Generation:
- POST /plans/generate-async returns 202 Accepted immediately
- Duplicate call while generating returns 409 Conflict
- GET /plans/agent/{agent_id}/generation-status reports processing, then completed
- Database persists updated MigrationPlan with status draft and creates Version 1 snapshot
"""
...
from app.modules.migration_plans.migration_plans_services import GenerationTaskManager


def _make_test_ast() -> TransformationPlanAST:
    return TransformationPlanAST(
        target_database_type="postgresql",
        ai_explanation="Initial test plan AST async generation",
...
@pytest.mark.asyncio
async def test_async_plan_generation_background_workflow():
    ...
    # Clear in-memory task tracker
    GenerationTaskManager._tasks.clear()
    ...
    with patch("app.modules.migration_plans.migration_plans_services.AsyncSessionLocal", TestSession):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver") as client:
            ...
            res_u = await client.post("/api/v1/auth/register", json={
                "email": "async_gen@example.com", "password": "Password123!", "name": "Async Gen Tester"
            })
            assert res_u.status_code == 201

            res_agent = await client.post("/api/v1/agents", json={
                "name": "Async Gen Agent",
                "agent_identifier": "async_gen_agent_01",
                "version": "1.0.0",
                "data_sources": [
            ...
            # 2. Trigger Async Plan Generation
            with patch.object(llm_plan_generator, "generate", return_value=mock_ast):
                res_start = await client.post("/api/v1/plans/generate-async", json={
                    "agent_id": agent_id,
                ...
                assert job_data["agent_id"] == agent_id
                plan_id = job_data["plan_id"]

                # 3. Duplicate request must be rejected with 409 Conflict
                res_dup = await client.post("/api/v1/plans/generate-async", json={
                    "agent_id": agent_id,
                    "target_config": {"database_type": "postgresql"},
                })
                ...
                res_status = await client.get(f"/api/v1/plans/agent/{agent_id}/generation-status")
                ...
                assert status_data["plan"]["status"] == "draft"
                assert status_data["plan_id"] == plan_id

            # 5. Check versions list - should have version 1 as initial_ai_generation
            res_vers = await client.get(f"/api/v1/plans/{plan_id}/versions")
            assert res_vers.status_code == 200
            vers_list = res_vers.json()
            assert len(vers_list) == 1
            assert vers_list[0]["version_number"] == 1
            assert vers_list[0]["edit_type"] == "initial_ai_generation"
